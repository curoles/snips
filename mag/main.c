...
void drawSomething(void* context, int x0, int y0, int x1, int y1)
{
    ...
    int sz = 10 + rand() % (image->xres/2);
    Vertice v1 = {x1, y1}; Vertice v2 = {x1+sz,y1+sz};
    GRect_drawFilled(image, &pixel, v1, v2);
}
...
void render(Image* image)
{
    for (size_t row = 0; row < image->yres; row++)
    {
        for (size_t col = 0; col < image->xres; col++) {
            Image_row(image, row)[col].clr[0] = 255;//255;
            Image_row(image, row)[col].clr[1] = 255;//128;
            Image_row(image, row)[col].clr[2] = 255;//50;
        }
    }
    ...
    //GLine_drawFromTo(image, &line, 0, 0, 250, 250Re);
    ...
    HilbertCurve_visit(drawSomething, image, 0, 0, 500, 0, 0, 500, 5);
}
